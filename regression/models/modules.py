...
__all__ = ['PoolingEncoder', 'PoolingSpinEncoder', 'CrossAttnEncoder', 'CrossAttnSpinEncoder', 'Decoder', 'build_mlp']
...
class PoolingSpinEncoder(PoolingEncoder):
    def __init__(self, dim_x=1, dim_y=1,
                 dim_hid=128, dim_lat=None,
                 pre_depth=4, post_depth=2,
                 use_H_A=False,
        ...
        dim_post = H_A_dim if use_H_A else dim_hid

        self.net_post = build_mlp(dim_post, dim_post, 2 * dim_lat if self.use_lat else dim_post, post_depth)


class CrossAttnSpinEncoder(nn.Module):
    def __init__(self, dim_x=1, dim_y=1, dim_hid=128, dim_lat=None, v_depth=4, qk_depth=2,
                 use_H_A=False,
                 use_ABLA_induce=True,
                 H_A_dim=64,
                 latent_dim_mult=1,
        ...
        self.net_q = build_mlp(dim_x, dim_hid, H_A_dim if use_H_A else dim_hid, qk_depth)

        self.attn = MultiHeadAttn(H_A_dim if use_H_A else dim_hid,
                                  H_A_dim if use_H_A else dim_hid,
                                  H_A_dim if use_H_A else dim_hid,
                                  2 * dim_lat if self.use_lat else dim_hid)
    ...
    def forward(self, xc, yc, xt, mask=None):
        ...
        v = self.spin(v, mask=mask)

        out = self.attn(q=q, k=v, v=v, mask=mask)
        if self.use_lat:
