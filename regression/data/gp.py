...
class GPSampler(object):
    ...
    def sample(self,
        ...
        # [B,N,Dx=1]
        batch.x = x_range[0] + (x_range[1] - x_range[0]) * torch.rand([batch_size, num_points, 1], device=device)
...
class RBFKernel(object):
    ...
    def __call__(self, x):
        length = 0.1 + (self.max_length-0.1) * torch.rand([x.shape[0], 1, 1, 1], device=x.device)
        scale = 0.1 + (self.max_scale-0.1) * torch.rand([x.shape[0], 1, 1], device=x.device)
        ...
        cov = scale.pow(2) * torch.exp(-0.5*dist.pow(2).sum(-1)) + self.sigma_eps**2 * torch.eye(x.shape[-2]).to(x.device)
...
class Matern52Kernel(object):
    ...
    def __call__(self, x):
        ...
        cov = scale.pow(2)*(1 + math.sqrt(5.0)*dist + 5.0*dist.pow(2)/3.0) * torch.exp(-math.sqrt(5.0) * dist) + self.sigma_eps**2 * torch.eye(x.shape[-2]).to(x.device)
...
class PeriodicKernel(object):
    ...
    def __call__(self, x):
        ...
        length = 0.1 + (self.max_length-0.1) * torch.rand([x.shape[0], 1, 1], device=x.device)
        scale = 0.1 + (self.max_scale-0.1) * torch.rand([x.shape[0], 1, 1], device=x.device)

        dist = x.unsqueeze(-2) - x.unsqueeze(-3)
        cov = scale.pow(2) * torch.exp(- 2*(torch.sin(math.pi*dist.abs().sum(-1)/p)/length).pow(2)) + self.sigma_eps**2 * torch.eye(x.shape[-2]).to(x.device)
