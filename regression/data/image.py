...
def img_to_task(img,
    ...
    batch = AttrDict()
    num_ctx = num_ctx or torch.randint(low=min_num_ctx, high=max_num_ctx+1, size=[1]).item()
    num_tar = num_tar or (num_pixels - num_ctx if target_all
                          else torch.randint(low=min_num_tar, high=max_num_tar+1, size=[1]).item())
...
def coord_to_img(x, y, shape):
    ...
    for b in range(B):
        for c in range(3):
            img[b, c, x1[b], x2[b]] = y[b, :, min(c, C-1)]
